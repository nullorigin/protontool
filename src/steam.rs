...
pub fn find_steam_installations() -> Vec<SteamInstallation> {
    ...
    if let Ok(steam_dir) = std::env::var("STEAM_DIR") {
        ...
        if path.join("steamapps").exists() {
            installations.insert(0, SteamInstallation {
...
fn parse_app_manifest(manifest_path: &Path, common_path: &Path, lib_path: &Path, steam_root: &Path) -> Option<SteamApp> {
...
pub fn find_proton_app(
    ...
    if let Some(ref version_name) = proton_version {
        selected_proton = steam_apps.iter()
...
fn find_compat_tool_for_app(config_vdf: &VDFDict, appid: u32) -> Option<String> {
    let software = config_vdf.get_dict("InstallConfigStore")?
        .get_dict("Software")?;
    
    let valve = software.get_dict("Valve")
        .or_else(|| software.get_dict("valve"))?;
    
    let steam = valve.get_dict("Steam")
